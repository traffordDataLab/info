## Ethnicity: multivariate density dot map of broad ethnic groups in Trafford (GIF)
...
# load R packages  ---------------------------
library(tidyverse) ; library(sf) ; library(gganimate) 
...
p <- ggplot() +
  ...
  geom_point(aes(x, y, color = ethnic_group, frame = ethnic_group), size = 0.01, alpha = 0.5, data = plot_dots) +
...
# save plot / data  ---------------------------
gganimate(p, title_frame = FALSE, filename = "output/figures/fig3.gif", ani.width = 650, ani.height = 650)
