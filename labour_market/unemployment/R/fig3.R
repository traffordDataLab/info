...
# load data  ---------------------------
# March 2018, 2011 Census Frozen Wards within Trafford, Age quinary, Claimant Count, Total
...
# plot data ---------------------------
p <- ggplot(results, aes(x = area_name, y = percent, width = total, fill = age)) +
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        axis.text.x = element_text(size = 8, hjust = 1, vjust = 0.4, angle = 90),
...
# save plot / data  ---------------------------
ggsave(file = "fig3.svg", width = 6, height = 5)

p + labs(title = "Age group of claimants in Trafford by wards, March 2018*",
...
ggsave(file = "fig3.png", width = 9, height = 8)

results <- results %>%
  select(-total) %>%
  mutate(percent = round(percent,1))

write_csv(results, "fig3.csv")
