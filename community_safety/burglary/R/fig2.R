## Burglary: time series with trend ##
...
ggplot(data = results, aes(month, value, colour = type, group = type)) +
  ...
  scale_y_continuous(position = "right", limits = c(0, 200)) + # change axis limits
...
results %>% 
  ...
  mutate(category = "Burglary", area_code = "E08000009", area_name = "Trafford") %>% 
