## Burglary: proportion of total crime (excluding ASB)##
...
gm <- read_csv("https://github.com/traffordDataLab/open_data/raw/master/police_recorded_crime/data/gm.csv.gz") %>% 
  filter(month == max(df$month) & category != "Anti-social behaviour") %>% 
  group_by(category) %>%
  summarise(n = n()) %>% 
  arrange(desc(n)) %>%
...
results <- df %>% 
  ...
  add_row(area_code = "E08000009", area_name = "Trafford", category = "Burglary", n = trafford$n, percent = trafford$percent) %>% 
  add_row(area_code = "E47000001", area_name = "Greater Manchester", category = "Burglary", n = gm$n, percent = gm$percent)
...
ggplot(results, aes(percent, area_name)) +
  ...
  scale_x_continuous(labels = function(x){ paste0(x, "%") }, limits=c(0, 35), expand = c(0,0)) +
