...
tab1 <- df %>% 
  ...
  left_join(., hlds, by = "area_name") %>% 
  mutate(rate = round((n/households)*1000,1)) %>% 
  select(Ward = area_name, Count = n, Rate = rate) %>% 
  add_row(Ward = "Trafford", Count = sum(.$Count), Rate = round((Count/94484)*1000,1)) %>% 
  add_row(Ward = "Greater Manchester", Count = NA, Rate = round((NA/1128066)*1000,1))
View(tab1)
write_csv(tab1, "outputs/tab1.csv")
