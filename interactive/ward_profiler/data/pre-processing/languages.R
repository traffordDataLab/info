...
# load data ---------------------------
df <- read_csv("http://www.nomisweb.co.uk/api/v01/dataset/NM_525_1.data.csv?date=latest&geography=E05000819...E05000839&rural_urban=0&cell=1...103&measures=20100&select=date_name,geography_name,geography_code,cell_name,obs_value")

# tidy data ---------------------------
languages <- df %>% 
  select(period = DATE_NAME, area_code = GEOGRAPHY_CODE, area_name = GEOGRAPHY_NAME, language = CELL_NAME, n = OBS_VALUE) %>% 
