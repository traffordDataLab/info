...
house_prices_aggregated <- read_csv("data/housing/house_prices_aggregated.csv")
house_prices <- read_csv("data/housing/house_prices.csv")
housing_type <- read_csv("data/housing/housing_type.csv")
housing_size <- read_csv("data/housing/housing_size.csv")
...
output$housing_text <- renderText({ 
  ...
  transactions <- prettyNum(select(filter(house_prices_aggregated, area_name == input$ward), transactions), big.mark = ",", scientific = FALSE)
  median_house_price <- prettyNum(select(filter(house_prices_aggregated, area_name == input$ward), median_price), big.mark = ",", scientific = FALSE)
  min_price <- prettyNum(select(filter(house_prices_aggregated, area_name == input$ward), min_price), big.mark = ",", scientific = FALSE)
  max_price <- prettyNum(select(filter(house_prices_aggregated, area_name == input$ward), max_price), big.mark = ",", scientific = FALSE)
...
output$housing_prices <- renderPlot({
  ...
  ward <- filter(house_prices, area_name == input$ward) %>% 
